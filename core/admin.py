...
from .models import (
    ...
    Review, Trip, UserFavorite, User, Contact
)
...
# ⭐ Place Resource با gallery
class PlaceResource(resources.ModelResource):
    gallery_str = fields.Field(attribute='gallery', column_name='gallery')
    ...
    class Meta:
        ...
        fields = (
            ...
            'gallery_str',   # ⭐
            'phone', 'contact_info', 'rating_avg', 'visit_count',
            'is_active', 'is_featured'
        )
        export_order = fields

    def dehydrate_gallery_str(self, place):
        """تبدیل لیست به رشته برای خروجی"""
        if place.gallery:
            import json
            return json.dumps(place.gallery, ensure_ascii=False)
        return '[]'
...
class RouteResource(resources.ModelResource):
    class Meta:
        ...
        fields = ('id', 'name', 'slug', 'category', 'origin', 'destination',
                  'route_type', 'duration_minutes', 'distance_km',
